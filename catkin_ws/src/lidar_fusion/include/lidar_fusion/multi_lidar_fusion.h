...
private:
    ...
    using InputPointCloud = pcl::PointCloud<PointXYZIRT>;
    using FusedPointCloud = pcl::PointCloud<PointXYZIRTS>;
    using SyncPolicy2 =
    ...
    struct Config {
        ...
        std::string diagnostics_topic = "/diagnostics/lidar_fusion";
        ...
        bool allow_missing_ring_time = false;
        bool enable_voxel_filter = false;
        double voxel_leaf_size = 0.05;
        bool enable_diagnostics = true;
        double diagnostics_period = 1.0;
        double overlap_pair_distance = 0.10;
        int overlap_max_points = 0;
        double overlap_warn_rmse = 0.08;
        double overlap_fail_rmse = 0.15;
    };
    ...
    struct Diagnostics {
        ...
        uint64_t legacy_xyzi_clouds = 0;
        uint64_t legacy_xyzi_points = 0;
        ...
        double overlap_rmse = 0.0;
        double overlap_max = 0.0;
        std::string overlap_status = "insufficient";
    };
    ...
    bool transformCloudToOutputFrame(const CloudConstPtr& cloud,
                                     ...
                                     uint16_t sensor_id,
                                     FusedPointCloud& transformed_cloud);
    bool fuseTransformedClouds(const std::vector<FusedPointCloud::Ptr>& clouds,
                               const ros::Time& output_stamp,
                               CloudMsg& output_msg);
    void updateOverlapDiagnostics(const FusedPointCloud& fused_cloud);
    void applyVoxelFilter(FusedPointCloud::Ptr& cloud);
