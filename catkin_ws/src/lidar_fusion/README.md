...
- 可选 voxel filter。
- 周期性输出 diagnostics，便于现场排查同步、TF、空点云、点数异常和三雷达重叠区一致性。
...
```text
catkin_ws/src/lidar_fusion/config/multi_lidar_fusion_timoo.yaml
catkin_ws/src/lidar_fusion/config/multi_lidar_fusion_tmlidar.yaml
```
...
multi_lidar_fusion:
  ...
  diagnostics_topic: /diagnostics/lidar_fusion
  enable_diagnostics: true
  diagnostics_period: 1.0
  overlap_pair_distance: 0.10
  overlap_warn_rmse: 0.08
  overlap_fail_rmse: 0.15
```
...
- `overlap_pair_distance`：重叠区诊断的近邻搜索距离，默认 `0.10 m`。节点以 `sensor_id=0` 的中心雷达为参考，分别统计其他雷达的最近跨传感器点距。
- `overlap_warn_rmse` / `overlap_fail_rmse`：重叠区 RMSE 诊断阈值，输出到 `/diagnostics/lidar_fusion` 的 `overlap_status` 字段。该诊断用于发现外参、同步或遮挡异常，不直接触发丢帧。

## 编译

在本仓库统一 catkin 工作空间中编译：
...
如果该包被复制到其他 catkin 工作空间，则仍应在对应工作空间根目录执行 `catkin_make`。
...
开启 `enable_diagnostics` 后，节点会周期发布 `diagnostics_topic`，并同步输出日志。诊断载荷使用分号分隔键值格式，便于 rosbag/HIL 回放工具解析：
...
这些诊断 topic 和日志应和 rosbag 一起保存，建议现场录制：

```bash
rosbag record \
  /lidar1/timoo_points /lidar2/timoo_points /lidar3/timoo_points \
  /points_raw /diagnostics/lidar_fusion /tf /tf_static /imu/data
```
