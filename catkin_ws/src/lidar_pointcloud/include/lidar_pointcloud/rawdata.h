...
{
  return readLittleEndianUint16(reinterpret_cast<const uint8_t*>(&block.header));
}

static inline uint16_t rawBlockRotation(const raw_block_t& block)
{
  return readLittleEndianUint16(reinterpret_cast<const uint8_t*>(&block.rotation));
}

static inline uint16_t rawBlockDistance(const raw_block_t& block, int data_offset)
{
