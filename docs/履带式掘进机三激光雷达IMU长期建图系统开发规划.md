...
| 本轮软件补强 | v5.15 将 `lidar_pointcloud` 与 `timoo_pointcloud` 原始 packet 的 `header/rotation/distance` 读取显式收敛到 `readLittleEndianUint16()` 与 `rawBlock*()` helper，不再依赖宿主机端序或 `two_bytes` union；新增 `lidar_rawdata_endian_test` 并扩展 `timoo_rawdata_tm16_test`，覆盖上/下 bank、block rotation 和距离字段的小端 wire bytes 解析。该项是离线可判定的原始包字节序软件补强，不替代真实 packet 状态字段实包确认或 TM-16 现场 rotation 统计 |
| 本轮软件补强 | v5.14 将 `timoo_pointcloud` TM-16 相邻 block azimuth 差值解析抽出为 `resolveTm16AzimuthDiff()`：首块 `35990 -> 10` 这类可信模 36000 rollover 不再因为缺少上一块差值而整块跳过；过大的反向跳变仍保守使用上一块差值，若上一块不可用则拒绝该 block。新增 `timoo_rawdata_tm16_test` 覆盖可信 rollover、异常反跳回退和无上一块时拒绝三条路径。该项是离线可判定的首块 rollover 策略补强，不替代真实 TM-16 packet rotation 统计或状态字段实包确认 |
| 本轮软件补强 | v5.13 将 `lidar_driver` 与 `timoo_driver` UDP `socket()`/`bind()` 失败路径从裸 `perror()` 改为带端口和 `errno` 文本的 `ROS_ERROR_STREAM`，让现场网口、权限或端口占用故障进入 ROS 日志链路；行为仍保持初始化失败后返回，不替代实机网卡名、权限和 UDP 包流入验证 |
