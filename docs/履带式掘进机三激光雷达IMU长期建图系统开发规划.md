...
| 本轮软件补强 | v5.14 将 `timoo_pointcloud` TM-16 相邻 block azimuth 差值解析抽出为 `resolveTm16AzimuthDiff()`：首块 `35990 -> 10` 这类可信模 36000 rollover 不再因为缺少上一块差值而整块跳过；过大的反向跳变仍保守使用上一块差值，若上一块不可用则拒绝该 block。新增 `timoo_rawdata_tm16_test` 覆盖可信 rollover、异常反跳回退和无上一块时拒绝三条路径。该项是离线可判定的首块 rollover 策略补强，不替代真实 TM-16 packet rotation 统计、大小端平台复核或状态字段实包确认 |
